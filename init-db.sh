...
psql -v ON_ERROR_STOP=1 --username "$POSTGRES_USER" --dbname "$POSTGRES_DB" <<-EOSQL
    SELECT 'CREATE DATABASE fepa_blog'
    WHERE NOT EXISTS (SELECT FROM pg_database WHERE datname = 'fepa_blog')\gexec
    GRANT ALL PRIVILEGES ON DATABASE fepa_blog TO fepa;
EOSQL

psql -v ON_ERROR_STOP=1 --username "$POSTGRES_USER" --dbname "fepa_blog" <<-EOSQL
